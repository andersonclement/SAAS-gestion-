...
@section('content')
    ...
    <div class="card">
        <h2 style="margin-top:0;">{{ __('Stock faible') }} ({{ $stockFaible->count() }})</h2>
        @if ($stockFaible->isEmpty())
            <p>{{ __('Aucun produit sous le seuil.') }}</p>
        @else
            <table>
                <thead><tr><th>{{ __('Boutique') }}</th><th>{{ __('Produit') }}</th><th>{{ __('Quantité') }}</th><th>{{ __('Seuil') }}</th></tr></thead>
                <tbody>
                    @foreach ($stockFaible as $entree)
                        <tr>
                            ...
                            <td>{{ $entree['produit']->seuil_alerte }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
